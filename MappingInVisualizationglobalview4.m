clear all;
close all;
% load('groundtruth_homography_667.mat');
% Load soccer images.
...
% ntemp =xlsread('ntemp1.xlsx');r = -5 + (5+5)*rand(10,1);

error_bound = [-0.0001 0.0002];
...
for i = 532:3:802 %532%:3:965%3:1000   %70:3:100
...
%% given frame and top view visualization correspondence
    AA5= U';
    dummy= ones(1,m);
    AA5=[AA5;dummy];
     ...
     %% homography from panorama to top view with error
    [sz1 sz2]= size(correp);
%error_coorep

r = -0.0001 + (0.0003)*rand(3);
error_correp = correp + r;
relative_error_registration_map =  norm(r)/ norm(correp);

  %% error top view point
  
     newpt_error= homography_transform(AA5,error_correp);
     
     
%  %% given frame and visualization correspondence- frme 667 groundtruth
%      gtruth_registration_map = correp_gt;
%      newpt_gt= homography_transform(AA5,gtruth_registration_map);
%      
     
     
     
     %% relative player registration error 
     [esize1 esize2]= size(newpt1);
     test_error_output= newpt1 -  randi([1 30],[esize1 esize2]);
      
     relative_error= newpt1 - test_error_output;
     relative_error1= sqrt(relative_error(:,1).^2 + relative_error(:,2).^2);
     norm_values = sqrt(newpt1(:,1).^2 + newpt1(:,2).^2);
     relative_error2= relative_error1./norm_values;
     proposed_error = sum(relative_error2)/esize1;
     proposed_error1=[proposed_error1 proposed_error]; 
     
%topview_points{i}=newpt1;
...
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
 ...
 %   plot(newpt_error(1,:),newpt_error(2,:),'or')
     title('Fig. (d): Top view visualization','fontsize', 18) 
    hold off 
  
   
 fname = 'D:\SAMRIDDHA\Research_Activities\panoramic view\Top View visualization\Topviewvideos2';
        filename= num2str(i);
